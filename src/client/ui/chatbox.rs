...
use super::bubble::BubbleResource;
...
fn key_events(
  ...
  mut bubble_res: ResMut<BubbleResource>,
) {
...
fn update(
  ...
  new_window(ctx.ctx_mut(), &s, "Chat Box", frame, pos, size, |ui| {
    ...
    */
    ...
    // ui.label("world");
    if chat_res.is_chat_mode {
      ui.text_edit_singleline(&mut chat_res.text).request_focus();
    } else {
      ui.text_edit_singleline(&mut chat_res.text);
    }
